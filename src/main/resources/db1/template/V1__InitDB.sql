...
alter table if exists department_doc
    add constraint FKijcc4gueupoqimflk9nk9e192
    foreign key (department_id)
    references department;

alter table if exists
    doc_employees add constraint
    FK3vdxj0r8c852c8b9xkyt5i4f6
    foreign key (employees_key)
    references employee;

alter table if exists
    doc_employees add constraint
    FKsyyqliyi4lwplnmveakvhsrq6
    foreign key (department_doc_docid)
    references department_doc;

alter table if exists employee add constraint
    FK6imirm13gp5u48ewsmuawj9u foreign key (id)
    references department;
